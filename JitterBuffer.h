...
{
public:
    JitterBuffer(MediaStreamItf* out, std::uint32_t step);
    ~JitterBuffer();
...
private:
    ...
    Status GetInternal(jitter_packet_t* pkt, unsigned int offset, bool advance);
    void Advance();
    ...
    HistoricBuffer<std::uint32_t, 64, double> m_delayHistory;
